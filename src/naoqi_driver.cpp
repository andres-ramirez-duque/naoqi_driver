...
#include "services/set_wakeup.hpp"
#include "services/set_leds.hpp"
#include "services/set_animation.hpp"
...
void Driver::registerDefaultConverter()
{
  ...
  action_chain_msg_.reset<tobo_planner::action_chain>( new tobo_planner::action_chain() );
  ...
  if ( head_enabled )
  {
    ...
    /****************************************** End Speech Event *************************************************/
  if ( end_speech_enabled )
  {
    std::string end_speech_events="ALAnimatedSpeech/EndOfAnimatedSpeech";
    //end_speech_events.push_back("ALAnimatedSpeech/EndOfAnimatedSpeech"); //ALAnimatedSpeech/EndOfAnimatedSpeech()
    boost::shared_ptr<EndSpeechEventRegister> event_register =
      boost::make_shared<EndSpeechEventRegister>( end_speech_events, sessionPtr_, action_chain_msg_);
    insertEventConverter("end_speech", event_register);
    if (keep_looping) {
      event_map_.find("end_speech")->second.startProcess();
    }
    //if (publish_enabled_) {
...
void Driver::registerDefaultSubscriber()
{
  ...
  registerSubscriber( boost::make_shared<naoqi::subscriber::AnSpeechSubscriber>("animated_speech", "/animated_speech", sessionPtr_, action_chain_msg_) );
}
...
void Driver::registerDefaultServices()
{
  ...
  registerService( boost::make_shared<service::SetWakeupService>("set_wakeup", "/naoqi_driver/set_wakeup", sessionPtr_) );
  registerService( boost::make_shared<service::SetLedsService>("set_leds", "/naoqi_driver/set_leds", sessionPtr_) );
  registerService( boost::make_shared<service::SetAnimationService>("set_animation", "/naoqi_driver/set_animation", sessionPtr_) );
