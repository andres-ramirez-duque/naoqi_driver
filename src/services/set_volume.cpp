...
namespace service
{
  ...
  bool SetVolumeService::callback( naoqi_bridge_msgs::SetFloatRequest& req, naoqi_bridge_msgs::SetFloatResponse& resp )
  {
    if (req.data < 0.0 or req.data > 100.0){
      resp.success = false;
      resp.message = "Volume data out of boundaries";
      return resp.success;
    }
    else{
      resp.success = helpers::driver::setVolume(session_, req);
      resp.message = "Volume data adjusted";
      return resp.success;
    }
  }
